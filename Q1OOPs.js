...
//c) Write a method getPG, which takes an array of base type Movie as its argument, and returns 
   //a new array of only those movies in the input array with a rating of "PG". You may assume the 
   //input array is full of Movie instances. The returned array need not be full.

   getAllMovies(film){
    return FileSystem.filter((v) => v.no.ofMovies === 2)
   };

   const Jailer = new movie ("Jailer","PG13");
   const Leo = new movie ("Leo","PG");
   const Jawan = new movie ("Jawan","PG15");

   console.log(Jailer);
   console.log(Leo);
   console.log(Jawan);
   console.log(Leo.getAllMovies([Jailer,Leo,Jawan]));
